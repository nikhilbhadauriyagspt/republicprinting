      ...
      <div className="flex items-end justify-between mb-16">
        <div className="space-y-3">
          ...
          <h2 className="text-3xl md:text-5xl font-bold text-slate-950 capitalize  leading-none">
            Best <span className="text-[#0096d6] italic">Sellers.</span>
          </h2>
        </div>
            ...
            >
              <div className="relative aspect-square rounded-[2rem] bg-slate-50 border border-slate-100 overflow-hidden flex items-center justify-center p-8 transition-all duration-700 hover:bg-white hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.05)] hover:border-blue-500/20">
                ...
                <button
                  ...
                  className={`absolute bottom-5 right-5 h-10 w-10 rounded-2xl flex items-center justify-center transition-all shadow-xl z-10 ${addedItems[p.id] ? 'bg-[#0096d6]/200 text-white' : 'bg-slate-950 text-white hover:bg-[#0096d6] scale-0 translate-y-4 group-hover:scale-100 group-hover:translate-y-0'}`}
                >
              ...
              <div className="mt-5 px-1">
                <Link to={`/product/${p.slug}`}>
                  <h3 className="font-bold text-slate-900 text-sm mb-1 capitalize  line-clamp-1 group-hover:text-[#0096d6] transition-colors">{p.name}</h3>
                </Link>
